...
for factor_select in factor_list:
    ...
    for scale_factor in scale_list:
        ...
        for month_id in range(0, 12):
            ...
            arr_list = [fvc_arr,cog,wf,ef,scf,kprime,wind_erosion_m]
            indices = pd.Index(["fvc", "cog", "wf", "ef", "scf", "kprime", "SL"])

            #Computing Correlations
            for i in range(len(arr_list)):
                for j in range(len(arr_list)):
                    new_min, new_max, new_min_index, new_max_index = corr_max_min(arr_list[i],arr_list[j],df_min[indices[i]][indices[j]], df_max[indices[i]][indices[j]],df_min_index[indices[i]][indices[j]], df_max_index[indices[i]][indices[j]], month_id)
                    #print(df_min[indices[i]][indices[j]], df_max[indices[i]][indices[j]])
                    df_min[indices[i]][indices[j]] = new_min
                    df_max[indices[i]][indices[j]] = new_max
                    df_min_index[indices[i]][indices[j]] = new_min_index
                    df_max_index[indices[i]][indices[j]] = new_max_index
                    #print(df_min)

            if en_hist_plot:
                row = int(month_id / 3)
                col = month_id % 3
                axes_hist_wf[row, col].hist(wf.ravel(), bins= HIST_BINS, range=(0.0, 2.0))
                axes_hist_ef[row, col].set_title('wf:'+ month_id_str, fontdict={'fontsize':'16'});

                axes_hist_ef[row, col].hist(ef.ravel(), bins = HIST_BINS, range=(0.0, 0.45))
                axes_hist_ef[row, col].set_title('ef:'+ month_id_str, fontdict={'fontsize':'16'});

                axes_hist_k[row, col].hist(kprime.ravel(), bins = HIST_BINS, range=(0.0, 1.0))
                axes_hist_k[row, col].set_title('k:'+ month_id_str, fontdict={'fontsize':'16'});

                axes_hist_scf[row, col].hist(scf.ravel(), bins = HIST_BINS, range=(0.0, 0.5))
                axes_hist_scf[row, col].set_title('scf:'+ month_id_str, fontdict={'fontsize':'16'});

                axes_hist_cog[row, col].hist(cog.ravel(), bins = HIST_BINS, range=(0.0, 1.0))
                axes_hist_cog[row, col].set_title('cog:'+ month_id_str, fontdict={'fontsize':'16'}); 

            if en_corr_plot:
                row = int(month_id / 3)
                col = month_id % 3
                axes_corr_wf[row, col].scatter(wf,wind_erosion_m)
                plot_title = 'wf:'+ month_id_str+ ', corr coeff = '+ str(compute_corr(wf,wind_erosion_m))
                axes_corr_wf[row, col].set_title(plot_title, fontdict={'fontsize':'16'}); 

                axes_corr_ef[row, col].scatter(ef,wind_erosion_m)
                plot_title = 'ef:'+ month_id_str+ ', corr coeff = '+ str(compute_corr(ef,wind_erosion_m))
                axes_corr_ef[row, col].set_title(plot_title, fontdict={'fontsize':'16'}); 

                axes_corr_k[row, col].scatter(kprime,wind_erosion_m)
                plot_title = 'kprime:'+ month_id_str+ ', corr coeff = '+ str(compute_corr(kprime,wind_erosion_m))
                axes_corr_k[row, col].set_title(plot_title, fontdict={'fontsize':'16'}); 

                axes_corr_scf[row, col].scatter(scf,wind_erosion_m)
                plot_title = 'scf:'+ month_id_str+ ', corr coeff = '+ str(compute_corr(scf,wind_erosion_m))
                axes_corr_scf[row, col].set_title(plot_title, fontdict={'fontsize':'16'}); 

                axes_corr_cog[row, col].scatter(cog,wind_erosion_m)
                plot_title = 'cog:'+ month_id_str+ ', corr coeff = '+ str(compute_corr(cog,wind_erosion_m))
        ...
        #print SL for each factor
        #print(master_name_list[factor_select]+':'+"{0:2.2f}, {1:2.2f}".format(scale_factor, SL_avg))
    ...
    if SL_sum_0p5_Exists and SL_sum_1p5_Exists:
        SL_sum_diff_0p5 = 100*((SL_sum_0p5-SL_sum_1p0)/SL_sum_1p0)
        SL_sum_diff_1p5 = 100*((SL_sum_1p5-SL_sum_1p0)/SL_sum_1p0)

        SL_sum_diff_0p5_file_dir = 'data_UTM/output/SL_sensitivity/'
        SL_sum_diff_0p5_file_path = os.path.join(SL_sum_diff_0p5_file_dir, master_name_list[factor_select] + '_diff_0p5.tif')
        RasterSave(SL_sum_diff_0p5, SL_sum_diff_0p5_file_path, dem_file_path)

        SL_sum_diff_1p5_file_dir = 'data_UTM/output/SL_sensitivity/'
        SL_sum_diff_1p5_file_path = os.path.join(SL_sum_diff_1p5_file_dir, master_name_list[factor_select] + '_diff_1p5.tif')
        RasterSave(SL_sum_diff_1p5, SL_sum_diff_1p5_file_path, dem_file_path)
    
    y_max = np.max([np.max(SL_avg_arr), np.max(SL_wo_veg_avg_arr), np.max(SL_fvc_100_avg_arr)])
    y_min = np.min([np.min(SL_avg_arr), np.min(SL_wo_veg_avg_arr), np.min(SL_fvc_100_avg_arr)])
    plt.xlim(0.4, 1.6)
    plt.plot(scale_list, SL_avg_arr, label="Actual Soil Loss")
    plt.plot(scale_list, SL_wo_veg_avg_arr, label="Soil Loss with no Vegetation Factor")
    if master_name_list[factor_select] != 'fvc':
        plt.plot(scale_list, SL_fvc_100_avg_arr, label="Soil Loss with 100% Vegetation")
        plt.plot(scale_list, SL_fvc_50_avg_arr, label="Soil Loss with 50% Vegetation")
    #plt.plot(scale_list, SL_fvc_10p_avg_arr, label="Soil Loss with +10% Vegetation")
    #plt.plot(scale_list, SL_fvc_20p_avg_arr, label="Soil Loss with +20% Vegetation")
    plt.legend(bbox_to_anchor=(1.04,1), loc="upper left")
    plt.ylabel("Soil Loss Sum (kg/m^2)")
    plt.xlabel("Input Parameter Scale Factor (n represents n*(original values))")
    plt.title(master_name_list[factor_select] + '_sensitivity')

    fig_file_dir = 'data_UTM/output/SL_sensitivity/'
    make_directory(fig_file_dir)
    fig_file_path = os.path.join(fig_file_dir, master_name_list[factor_select] + '_sensitivity.png')
